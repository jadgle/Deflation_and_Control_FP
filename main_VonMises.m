...
beta_m1 = 1; % phase transition between 0.1 and 0.01
beta= 1/beta_m1;


%%%%%%
% 
W = @(x) -exp(cos(x))/(2*pi*besselj(0, 1i)); %-exp(cos(x))/(2*pi*besselj(0, 1i)); 
V = @(x) 0;

%% Discretization

l = 70; %number of modes
...
set(gcf,'color','w');
t_interval = [0,15];
[t,a] = ode45(@(t,a) odefcn(t,a,A1,B,C1,M1) , t_interval , guess);
...
trying = 1;
%guess = u_unif;
while trying < 5 % this is trying>2 is useful when we are changing initial condition
    u = guess;
...
u00=[linspace(0.05,0,nt-1)*0;zeros(L-1,nt-1)];
X=zeros(L,nt); 
guess0 =rand(size(xi));
guess0 = guess0/(sum(guess0.*wi));
guess = coeffs(xi,wi,l,guess0);
...
[P0,dJ0]=IRKadj(X00,x0*0,u00,dt,backward,dbackward,gamma);  %backward pass
u0 = u00*0;

for k=1:nt-1
    u0(:,k) = u00(:,k) - 0.05*dJ0(:,k);
end
...
k=1;
for t = 1:15:nt
    norma_uncontrolled(k) = norm(X_uncontrolled(:,t)-a_infty)^2;
    k = k+1;
end
figure(2)
plot(1:15:nt,norma(1:k-1),'ro')
hold on
plot(1:15:nt,norma_uncontrolled(1:k-1),'b*')

%%
%[t,a] = ode45(@(t,a) odefcn(t,a,A1,B,C1,M1) , t_interval , x0);
%X_uncontrolled = a;
figure(3)
for n = 1:10:nt
    a = X(:,n);
