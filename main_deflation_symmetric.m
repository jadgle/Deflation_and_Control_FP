...
N = 100;
m1 = linspace(-1.1,1.1,N);
sol = zeros(N,1);

Z = @(m1) sum(exp(-beta*V(xi)+(beta*kappa*(m1*cos(xi)))).*wi);
rho = @(x,m1,Z) (1/Z(m1))*exp(-beta*V(x)+(beta*kappa*(m1*cos(x))));
freeE = zeros(size(m1));

Free_energy = @(x,w,m,rho,Z) sum(V(x).*rho(x,m,Z).*w) + sum(beta_m1*rho(x,m,Z).*log(rho(x,m,Z)).*w)-(kappa/2)*convol(x,w,m,rho,Z);
for i=1:N
    m_1 = m1(i);
    sol(i,:) = R(m_1,beta,alpha,kappa);
    freeE(i) = Free_energy(xi,wi,m_1,rho,Z);
end
plot(m1,sol,'k-', 'LineWidth', 3)
hold on
plot(m1,m1,'-.','Color',[0.8350 0.0780 0.1840],'LineWidth', 3)
hold on


exp_int_Z = @(a,x) exp(a*cos(2*pi*x));
S = [ones(n_gauss,1),beta*kappa*cos(xi)];
c = [];
...
for n = 1:n_deflation
    ...
    if sum(f<0)<1 && norm(F(u,A,B,C))<1e-3
        Y = log(f)+beta*V(xi);
        c(:,n) = inv(S'*S)*S'*Y;
        disp([num2str(k),'-th solution: ', '   ',num2str(norm(F(u,A,B,C))), '      ',mat2str(c(:,n)')])       
        k = k+1;
        plot(c(2,n)*ones(1,N),m1,':','LineWidth', 3)
